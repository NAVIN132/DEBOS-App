...
const generateDemo = async () => {
  ...
  RNSecureKeyStore.set('testkey', keys.private, {accessible: ACCESSIBLE.ALWAYS_THIS_DEVICE_ONLY})
    .then((res) => {
        console.log(res);
    }, (err) => {
        console.log(err);
...
class App extends Component {
  ...
  onClickListener = (viewId) => {
    ...
    {
       // const encodedMessage = await RSA.encrypt('1234', keys.public)
        //Alert.alert('encoded message:', encodedMessage);
  ...
  render () {
    return (
      <View>
        ...
        <TouchableHighlight  onPress={() => this.onClickListener('GENK1')}>
        <Text >Generate</Text>
      </TouchableHighlight>
      <TouchableHighlight  onPress={() => this.onClickListener('GENK2')}>
        <Text >Encrypt</Text>
      </TouchableHighlight>
      <TouchableHighlight  onPress={() => this.onClickListener('GENK3')}>
        <Text >Decrypt</Text>
      </TouchableHighlight>
      </View>
    )
